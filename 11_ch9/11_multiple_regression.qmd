...
It's too verbose to say "subpopulations with the same values of $X_2,\ldots,X_{p-1}$ but one unit difference in $X_1$ differ on average by $\beta_1$." So nobody says this in reports/papers.

Instead, they use the word "controlling" or "adjusting". I prefer "adjusting", but "controlling" is more common.

E.g., species with on-day longer gestations length have a mean brain weight $\beta_1$ grams larger *adjusting for body weight*.
:::
...
Some folks call this [Simpson's Paradox](https://en.wikipedia.org/wiki/Simpson%27s_paradox).
...
- If $H_0$ were true, $T \sim t_{\nu}$

- $\nu$ is the degrees of freedom, and is $n$ minus the number of parameters.

-   $p$-value is calculated from the tail area probability.
    ...
    plt_t(lb = 1.5, two_sided = TRUE) +
      ...
      scale_x_continuous(breaks = c(-1.5, 0, 1.5), labels = c("-T", "0", "T"))
    ```
