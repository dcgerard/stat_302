...
-   E.g., either survival ($Y_i = 1$) or death ($Y_i = 0$) of individual $i$ from the Donner Party of 1846.
    ...
    ```{r}
    #| message: false
    donner <- read_csv("https://dcgerard.github.io/stat_302/data/case2001.csv")
    glimpse(donner)
    ```
...
-   Another example is probit regression (more popular in econ) where $g(\mu_i) = \Phi^{-1}(\mu_i)$, the standard normal quantile function (`qnorm()`). This gives you almost identical results (except in the tails), but statisticians don't like the probit because it's less interpretable than the logit.
...
- **Exercise**: The nocturia data set, described [here](https://dcgerard.github.io/stat_415_615/data.html#Nocturia) and downloadable from [here](https://dcgerard.github.io/stat_415_615/data/nocturia.csv), contains patient covariates that are believed to be associated with whether the individual has nocturia (wakes up to pee). Download the data, the fit of logistic regression model of nocturia on age. Interpret the resulting coefficients.
...
|:--|:--|
| `Temperature` | Launch temperature (°F) |
| `Failure` | O-ring failure: `"Yes"` or `"No"` |
...
- (a) Make a scatterplot of `Temperature` (x-axis) vs `Failed` (y-axis). Does the relationship look like what you'd expect?

- (b) Fit a logistic regression of `Failed` on `Temperature`. Interpret the estimated slope coefficient as an odds ratio.

- (c) Use a drop-in-deviance test to test whether temperature is associated with O-ring failure.

- (d) Use `confglm()` to compute a 95% confidence interval for the probability of O-ring failure at 31°F. Based on this, should the Challenger have launched that morning?

---

**2.** @holst1988debate conducted a case-control study in The Hague, Netherlands, to investigate whether keeping pet birds is an independent risk factor for lung cancer after accounting for smoking and other variables. They recruited 49 lung cancer patients and 98 controls.

| Variable | Description |
|:--|:--|
| `LC` | `"LungCancer"` or `"NoCancer"` |
| `FM` | Sex: `"Female"` or `"Male"` |
| `SS` | Socioeconomic status: `"High"` or `"Low"` |
| `BK` | Birdkeeping: `"Bird"` or `"NoBird"` |
| `AG` | Age (years) |
| `YR` | Years of smoking prior to diagnosis/examination |
| `CD` | Average cigarettes smoked per day |
...
- (a) Is this a prospective or retrospective study? What does this imply for how you can interpret the results?

- (b) Fit a logistic regression of `LungCancer` on `BK`, `AG`, `YR`, `CD`, `FM`, and `SS`. Interpret the odds ratio for `BK`.

- (c) Use a drop-in-deviance test to evaluate whether birdkeeping is associated with lung cancer after adjusting for the other variables. State your conclusion in context.

- (d) Obtain and interpret a 95% confidence interval for the odds ratio associated with `BK`.
