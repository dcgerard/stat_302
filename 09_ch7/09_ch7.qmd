...
- According to Big Bang theory, the distance $Y$ between any two celestial
  ...
  $$Y = TX.$$
- Edwin Hubble measured recession velocities and distances for 24 nebulae
  beyond the Milky Way.
- If the Big Bang model is correct:
  - The **slope** of a regression of Distance on Velocity estimates the age of
    the universe.
...
- $\beta_1$ is the **slope** (rise over run), and
- $\beta_0$ is the **$y$-intercept** (the value of the line at $X = 0$).
...
The animation below illustrates assumptions 1 and 2:

- At each fixed velocity $x_0$, the distance values are normally distributed
  around the fitted line.
- The spread $\sigma$ is the same at every $x_0$.
...
- Three candidate lines are shown below.
- The vertical segments are the **residuals**.
...
The name "least *squares*" is literal:

- Each residual is the vertical distance from a point to the line.
- Draw a square whose side length equals that residual.
- The **area** of each square is a squared residual $\hat\varepsilon_i^2$.
- The **RSS is the total shaded area** — OLS is the line that makes it smallest.
...
#| fig-width: 8
#| fig-height: 5
sq_dat <- tibble(x = c(1, 2, 3), y = c(1.6, 1.3, 3.1))

make_square_plot <- function(dat, b0, b1, title) {
  dat <- dat |>
    mutate(fitted = b0 + b1 * x,
           resid  = y - fitted,
           side   = abs(resid))
  rss <- round(sum(dat$resid^2), 2)
  ggplot(dat, aes(x = x, y = y)) +
    geom_rect(aes(xmin = x, xmax = x + side,
                  ymin = pmin(y, fitted), ymax = pmax(y, fitted)),
              fill = "#E69F00", alpha = 0.4, color = "#E69F00") +
...
ols_sq <- lm(y ~ x, data = sq_dat)
p_poor <- make_square_plot(sq_dat, 3, -0.4, "Poor fit")
p_ols  <- make_square_plot(sq_dat, coef(ols_sq)[[1]], coef(ols_sq)[[2]],
                           "OLS fit")
p_poor | p_ols
```
...
As before, you can use `tidy()` and `glance()` to also get these:
...
- If we repeatedly drew new samples (keeping the $X_i$ values fixed) and
  re-estimated, the resulting estimates would vary.
...
- Blue = true regression line.
- Dashed red = estimated line from that simulated sample.
- The slope shifts across samples, but centers on the true $\beta_1$.
...
- To test $H_0 : \beta_1 = 0$ (no linear association):
  - Compute $t^* = \hat\beta_1 / SE(\hat\beta_1)$.
  - Find the two-sided $p$-value $2 P(t_{n-2} \leq -|t^*|)$.
...
- **Randomized experiment:** a one-unit increase in $X$ *causes* a
  $\hat\beta_1$ unit change in the mean of $Y$.
- **Observational study:** populations that differ by one unit in $X$
  differ by $\hat\beta_1$ units in $Y$ on average.
- For the nebula data (observational):
  - Nebulae with a recession velocity 1 km/s faster tend to be about 0.0014
    megaparsecs farther from Earth ($p < 0.001$, 95% CI: 0.00090 to 0.0018).
...
- The **prediction interval** is widest.
- The **pointwise confidence interval** is narrowest.
- The **simultaneous band** falls between.

---

# Extrapolation vs. Interpolation

- **Interpolation** means making estimates *within* the range of the observed
  $X$ values.
- **Extrapolation** means predicting *outside* that range.
...
::: {.callout-warning}
Correlation is always a useful **summary measure**.

Correlation is only an appropriate **inference** device if both $X$ and $Y$ are random. So **don't** use it for hypothesis testing and confidence intervals if $X$ is fixed.
:::
...
#| fig-height: 3
orange  <- "#E69F00"
aug701  <- augment(lmout)
ybar    <- mean(aug701$Distance)

seg_total <- data.frame(x = aug701$Velocity, xend = aug701$Velocity,
                        y = aug701$Distance, yend = ybar)
seg_resid <- data.frame(x = aug701$Velocity, xend = aug701$Velocity,
                        y = aug701$Distance, yend = aug701$.fitted)
seg_reg   <- data.frame(x = aug701$Velocity, xend = aug701$Velocity,
                        y = ybar, yend = aug701$.fitted)
line_df   <- data.frame(x = aug701$Velocity, y = aug701$.fitted)
...
- The nebula data are inconsistent with the simple $Y = TX$ Big Bang model (or
  there is systematic measurement error).
...
tidy(lm_noint, conf.int = TRUE)
```

- The estimated age is approximately 0.00192 megaparsec-seconds per km
  (95% CI: 0.00153 to 0.00232).
- Converting to years gives an estimate of roughly 1.9 billion years — far too
  young by modern standards (the universe is approximately 13.8 billion years
  old).
- This reflects the limited accuracy of Hubble's early distance measurements.
