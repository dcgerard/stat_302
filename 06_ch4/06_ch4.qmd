...
- If you are randomly assigning treatments to pairs of units (e.g., if we would have randomly assigned which baby in a pair gets formula and which gets breast feeding), then the differences under the null are theoretically symmetric about 0.
...
- The **pseudomedian** is the median of a distribution is the median of the distribution of $(X_1 + X_2)/2$ where $X_1$ and $X_2$ are iid from the original distribution.
...
-   E.g., from our effusion data, the number of positive and number of non-zero values are:
    ```{r}
    npos <- sum(effusion$diff > 0)
    npos
    n <- sum(effusion$diff != 0)
    n
    ```
