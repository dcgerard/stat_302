...
- More powerful alternative to Bonferroni, with applications in the exact same scenarios as Bonferroni.

-   We won't cover what it does exactly. But you should always use it over Bonferroni. (Holm the default in R's `p.adjust()`).

    ```{r}
    pvec <- c(0.001, 0.01, 0.03, 0.04, 0.05)
    ```
...
- **Conditions:** only valid for **all pairwise comparisons** in a one-way ANOVA.
- **Advantage:** smaller adjusted $p$-values than Bonferroni/Holm when goal is all pairwise tests.

- **Intuition**: 
  - Let $\bar{Y}_{\max}$ be the maximum mean of $I$ groups.
  - Let $\bar{Y}_{\min}$ be the min mean of $I$ groups.
  - If the sample sizes are all the same, then SE of each pairwise comparison is $s_{p}\sqrt{1/n + 1/n}$. Call this $\text{SE}$.
  - All pairwise tests reject when
    $$
    \frac{\bar{Y}_i - \bar{Y}_J}{\text{SE}} < C
    $$
    for some $C$.
  - Tukey-Kramer find the distribution of 
    $$
    \frac{\bar{Y}_{\max} - \bar{Y}_{\min}}{\text{SE}}
    $$
  - This is called the "Studentized Range Distribution". It's essentially how extreme the largest $t$-statistic (among $\binom{I}{2}$ pairwise comparisons) can get.
  - Quantiles of this distribution are used to find $C$ to control the family wise Type I error rate.
...
- Dunnett: All pairwise comparisons versus a single reference value. 
  - I.e., if $\mu_0$ is the reference mean, then it will calculate tests and confidence intervals for $\mu_0 - \mu_i$ for all other $i$.
  - More powerful than Tukey when you just care about reference comparisons.
  
- Scheffé: All possible contrasts.
  - Do as many tests as you want (planned or unplanned).
  - Very wide intervals.
