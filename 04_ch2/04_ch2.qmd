...
if (!file.exists("02_fig/fig_sampling_dist.gif")) {
  library(gifski)
  library(ggplot2)
  library(patchwork)
  ...
  for (fi in seq_along(frame_schedule)) {
    ...
    p_obs <- ggplot(obs_df) +
      geom_boxplot(mapping = aes(x = obs), outliers = FALSE) +
      geom_point(color = "#4a90d9", size = 3.2, alpha = 0.85,
                 mapping = aes(x = obs, y = ypos)) +
...
4.  Compare $Z$ to a $N(0, 1)$

    ```{r}
    #| echo: false
    plt_t(ub = -2, two_sided = TRUE) +
      geom_vline(xintercept = c(-2, 2), lty = 2, col = "#56B4E9")
    ```
...
- **Why use $t$ instead of $N(0,1)$?**
  ...
  - Using $N(0,1)$ would make you over-confident in the weirdness of
    $\bar{X}$.
...
data.frame(
  x    = c(xseq, xseq, xseq),
  y    = c(dt(xseq, df = 1), dt(xseq, df = 5), dnorm(xseq)),
  dist = rep(c("t (df=1)", "t (df=5)", "Normal"), each = length(xseq))
) |>
...
if (!file.exists("02_fig/fig_ci_interpretation.gif")) {
  library(gifski)
...
- XKCD (only half-joking):
  ...
  <img src="https://imgs.xkcd.com/comics/p_values.png" alt="XKCD graphic depicting p-value interpretationgs">
