...
- Explain the one-way ANOVA model for observational studies and randomized
  experiments.
- State and interpret the three types of hypotheses: pairwise, omnibus, and
  one-group-vs-rest.
...
The model equation $Y_{ij} = \mu_i + \varepsilon_{ij}$ looks the same in both
settings, but the randomized experiment has a useful additional interpretation.
In the diet study, the same individual mouse could in principle have been
assigned to any diet. If we call the response under diet 1 (N/N85, the
control) $Y^* = \mu_1 + \varepsilon_j$, then the same mouse under diet $i$
would have response:

$$Y_j^{(i)} = \mu_i + \varepsilon_j
             = (\mu_i - \mu_1) + \mu_1 + \varepsilon_j
             = \delta_i + Y^*$$

where $\delta_i = \mu_i - \mu_1$ is the **treatment effect** of diet $i$
relative to the control. Because diets were randomly assigned, $\delta_i$ has
a genuine causal interpretation: it is how much longer (or shorter) a mouse
would live if assigned to diet $i$ instead of the control.

In an observational study we cannot make this argument — the "same individual
under a different group" is a counterfactual we cannot observe, and
confounding may explain any observed difference.

The difference is in the conclusions:

- **Observational study** (Spock trial): use associational language
  ("is associated with", "differs between").
- **Randomized experiment** (diet study): use causal language
  ("causes", "results in", "affects").
...
**Why $\nu = n - I$?** Because $s_p^2$ uses *all $n$ observations across all
$I$ groups* to estimate $\sigma^2$ — not just the $n_i + n_j$ observations in
the two groups being compared. Using more data gives a better estimate, which
in turn gives larger degrees of freedom ($n - I$ instead of $n_i + n_j - 2$),
making the $t$ distribution closer to a standard normal and the test more
powerful.
...
- Full model residual: $Y_{ij} - \bar{Y}_{i\bullet}$ — how far observation
  $(i,j)$ is from *its group's mean*
- Reduced model residual: $Y_{ij} - \bar{Y}_{\bullet\bullet}$ — how far
  observation $(i,j)$ is from *the grand mean*
...
Each RSS adds up all the squared residuals across every observation. Think of
it as a scoreboard for how badly the model is fitting: a small RSS means
predictions are close to the data; a large RSS means they are far away. Both
RSS values involve squaring (so positive and negative residuals don't cancel)
and summing (so we get a single overall measure).

The **extra sum of squares** (ESS) measures how much the fit improves when
we go from the reduced model to the full model:

$$ESS = RSS_{reduced} - RSS_{full}, \quad df_{extra} = I - 1$$

**Why is $RSS_{reduced} \geq RSS_{full}$ always?** The full model has the
reduced model as a special case: if all the $\mu_i$ happen to be equal, the
full model's separate group means would all estimate the same value as the
grand mean, and the two models would give identical fits. In general, the
full model has more flexibility, so it can *never* fit worse than the reduced
model — it can always do at least as well. Therefore $ESS \geq 0$ always.

**What does ESS tell us?** If $H_0$ is true (all means really are equal),
fitting separate group means should not improve the fit by very much — the
data don't contain strong evidence for distinct groups. In that case, ESS
will be positive but small. If the means truly differ (the data come from
groups with different means), the full model will fit much better, and ESS
...
In more complex questions we compare **three** or more nested models. Each
model is a special case of the next:
...
Each $F$-test compares adjacent models. Crucially, **all F-tests use the
variance estimate $s_p^2 = RSS_{full}/df_{full}$ from the most general
(full) model**, regardless of which pair is being compared.
...
Note: the $F$-statistic in the table uses $s_p^2 = 1864/39 \approx 47.8$
(from the **full** model) as the denominator throughout.

::: {.callout-note}
## $s_p^2$ is not the same as the overall sample variance

The pooled variance $s_p^2 = RSS_{full}/39 = 1864/39 \approx 48$ measures
how much observations vary *within* groups — after accounting for differences
in group means. It is what ANOVA uses to estimate $\sigma^2$.

If you ignored the groups entirely and computed a single sample variance for
all 46 venires, you would get $s^2 = RSS_{reduced}/45 = 3792/45 \approx 84$.
This is larger because it lumps together within-group and between-group
variation — the judges have different means, and that extra spread inflates
$s^2$. Using $s_p^2$ removes the between-group contribution and focuses on
the within-group noise, giving a better estimate of $\sigma^2$.
:::
...
## Omnibus $F$-Test with `aov()` and `summary()`
...
```{r}
#| label: ggplot-resids
resid_vec  <- residuals(aout_full)
fitted_vec <- fitted(aout_full)
resid_df   <- data.frame(resid = resid_vec, fitted = fitted_vec,
                         Judge = case0502$Judge)

p_rv_fit <- ggplot(resid_df, aes(x = fitted, y = resid)) +
  geom_point() +
...
p_rv_grp <- ggplot(resid_df, aes(x = Judge, y = resid)) +
  geom_boxplot() +
  geom_hline(yintercept = 0, linetype = "dashed") +
...
resid_bad_var <- rnorm(n = nrow(case0502),
                       mean = 0,
                       sd   = fitted_vec^3 / 3e6)
ggplot(data.frame(resid = resid_bad_var, fitted = fitted_vec),
...
resid_out <- resid_vec
resid_out[1] <- 35
ggplot(data.frame(resid = resid_out, fitted = fitted_vec),
       aes(x = fitted, y = resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(x = "Fitted values", y = "Residuals",
