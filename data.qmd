...
- [sardine.csv](./data/sardine.csv): Vertebrae counts from 12,858 Pacific sardines (*Sardinops sagax*) sampled from six locations along the Pacific coast. [@clarke1936variations]
  - `location`: integer code for sampling location — `1` Alaska, `2` British Columbia, `3` San Francisco, `4` Monterey, `5` San Pedro, `6` San Diego
...
- [lsd.csv](./data/lsd.csv): Results from an experiment in which seven volunteers were administered LSD (lysergic acid diethylamide) and subsequently took a standardized math test. Used to study the relationship between drug concentration in the blood and cognitive performance.
  - `conc`: LSD concentration in blood tissue (micrograms per litre)
  - `score`: math test score (out of 100)
...
- [espresso.csv](./data/espresso.csv): Data from @masella2015new, a study of a new espresso brewing method in which pressure differentials (rather than steam) control water flow, allowing finer control of temperature and pressure. 
  - `foamIndx`: ratio of foam to liquid volume (expressed as a percentage) measured 30 seconds after extraction
  - `trt_id`: integer label for each treatment group (temperature × pressure combination)
  - `tempC`: water temperature in degrees Celsius, with levels `75`, `85`, and `95`
  - `prssBar`: brewing pressure in bar, with levels `15` and `20`

- [poker.csv](./data/poker.csv): Results from @meyer2013poker, a quasi-experimental study of poker skill. Players were classified by skill level and randomly dealt one of three hand qualities. The study investigated whether poker outcomes depend on skill, hand quality, and betting structure, or primarily on chance.
  - `Skill`: skill level of the player, with levels `"Expert"` and `"Average"`
  - `Hand`: quality of the cards dealt, with levels `"Bad"`, `"Neutral"`, and `"Good"`
  - `Limit`: betting structure, with levels `"Fixed"` (fixed-limit) and `"None"` (no-limit)
  - `Cash`: final cash balance (in dollars) at the end of the session
